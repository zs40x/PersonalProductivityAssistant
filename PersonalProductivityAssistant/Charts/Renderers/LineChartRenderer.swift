...
public class LineChartRenderer: LineRadarChartRenderer
{
    ...
    public func drawDataSet(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        {
            context.setLineDash(phase: dataSet.lineDashPhase, lengths: dataSet.lineDashLengths!, count: dataSet.lineDashLengths!.count)
        }
        else
        {
    ...
    public func drawCubicBezier(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        if (size - minx >= 2)
        {
            ...
            cubicPath.moveTo(&valueToPixelMatrix, x: CGFloat(cur.xIndex), y: CGFloat(cur.value) * phaseY)
            ...
            for j in stride(from: (minx + 1), to: min(size, entryCount), by: 1)
            {
                ...
                cubicPath.addCurve(&valueToPixelMatrix,
                                      cp1x: CGFloat(prev.xIndex) + prevDx,
                                      cp1y: (CGFloat(prev.value) + prevDy) * phaseY,
                                      cp2x: CGFloat(cur.xIndex) - curDx,
                                      cp2y: (CGFloat(cur.value) - curDy) * phaseY,
                                      endingAtX: CGFloat(cur.xIndex),
                                      y: CGFloat(cur.value) * phaseY)
            }
        }
    ...
    public func drawHorizontalBezier(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        if (size - minx >= 2)
        {
            ...
            for j in stride(from: (minx + 1), to: min(size, entryCount), by: 1)
            {
                ...
                cubicPath.addCurve(&valueToPixelMatrix,
                                      cp1x: cpx, cp1y: CGFloat(prev.value) * phaseY,
                                      cp2x: cpx, cp2y: CGFloat(cur.value) * phaseY,
                                      endingAtX: CGFloat(cur.xIndex), y: CGFloat(cur.value) * phaseY)
            }
        }
    ...
    public func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        if (dataSet.colors.count > 1)
        {
            ...
            for j in stride(from: minx, to: count, by: 1)
            {
                ...
                context.strokeLineSegments(between: _lineSegments, count: pointsPerEntryPair)
            }
        }
        else
        { // only one color per dataset
            ...
            if e1 != nil
            {
                ...
                if j > 0
                {
                    ...
                    context.strokeLineSegments(between: _lineSegments, count: size)
                }
            }
        }
    ...
    private func generateFilledPath(dataSet: ILineChartDataSet, fillMin: CGFloat, from: Int, to: Int, matrix: CGAffineTransform) -> CGPath
    {
        ...
        if e != nil
        {
            filled.moveTo(&matrix, x: CGFloat(e.xIndex), y: fillMin)
            filled.addLineTo(&matrix, x: CGFloat(e.xIndex), y: CGFloat(e.value) * phaseY)
        }
        ...
        for x in stride(from: (from + 1), to: Int(ceil(CGFloat(to - from) * phaseX + CGFloat(from))), by: 1)
        {
            ...
            if isDrawSteppedEnabled
            {
                ...
                filled.addLineTo(&matrix, x: CGFloat(e.xIndex), y: CGFloat(ePrev.value) * phaseY)
            }
        ...
        if e != nil
        {
            filled.addLineTo(&matrix, x: CGFloat(e.xIndex), y: fillMin)
        }
        filled.closeSubpath()
        ...
        for i in 0 ..< dataSets.count
        {
            ...
            for j in stride(from: minx, to: Int(ceil(CGFloat(maxx - minx) * phaseX + CGFloat(minx))), by: 1)
            {
                ...
                if drawTransparentCircleHole
                {
                    ...
                    // Cut hole in path
                    context.addArc(centerX: pt.x, y: pt.y, radius: circleHoleRadius, startAngle: 0.0, endAngle: CGFloat(M_PI_2), clockwise: 1)
